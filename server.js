...
const uri = "mongodb+srv://chowdaryaasritha95:Aasritha123@cluster0.ax0dh8o.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0";
...
// Middleware for MongoDB operations
app.use(async (req, res, next) => {
  try {
    await client.connect();
    req.db = client.db("Astrology");
    req.col = req.db.collection("Register");
    req.cont = req.db.collection("Contact");
    req.prof = req.db.collection("Profile");
    req.log = req.db.collection("Status");
    next();
  } catch (err) {
    console.error('Error connecting to MongoDB Atlas', err);
...
app.post('/', async (request, response) => {
  ...
  app.get('/check', async (request, response) => {
    ...
    try {
      const user = await col.findOne({ email: request.query.un });
      console.log(user);
      if (!user) {
      ...
      if (user.password === request.query.pw) {
        const existingLog = await log.findOne({ mail: user.email });
        if (existingLog) {
          await log.updateOne({ mail: user.email }, { $set: { stat: true } });
        } else {
          await log.insertOne({ mail: user.email, stat: true });
        }
        response.send("pass");
      } else {
